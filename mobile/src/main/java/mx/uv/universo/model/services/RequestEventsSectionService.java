...
import mx.uv.universo.interfaces.EventsSectionService;
import mx.uv.universo.model.entities.Sections;
import retrofit.RestAdapter;

public class RequestEventsSectionService extends IntentService {
    protected final static String ID="ID";

    public RequestEventsSectionService(String name) {
        super(name);
    }

    public static void callService(Activity activity, int id) {
        Intent intent = new Intent(activity, RequestEventsSectionService.class);
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        RestAdapter restAdapter = new RestAdapter.Builder()
                ...
                .build();
        EventsSectionService service=  restAdapter.create( EventsSectionService.class);
        boolean errorDownload =false;
        List<Object> articles =null;

        try {
            articles = service.getEventsSection(intent.getIntExtra(ID, 0));
