...
import mx.uv.universo.interfaces.GeneralSectionService;
import mx.uv.universo.model.entities.Sections;
import retrofit.RestAdapter;

public class RequestGeneralSectionService extends IntentService {
    protected final static String ID="ID";

    public RequestGeneralSectionService(String name) {
        super(name);
    }

    public static void callService(Activity activity, int id) {
        Intent intent = new Intent(activity, RequestGeneralSectionService.class);
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        RestAdapter restAdapter = new RestAdapter.Builder()
                ...
                .build();
        GeneralSectionService service=  restAdapter.create(GeneralSectionService.class);
        boolean errorDownload =false;
        List<Object> articles =null;

        try {
            articles = service.getGeneralSection(intent.getIntExtra(ID, 0));
